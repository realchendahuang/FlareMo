...
writeFileSync(
  ...
  [
    ...
    "4. The migrations and memo insert triggers rebuild the derived `memos_fts` index; do not restore FTS shadow tables.",
    "5. Restore R2 objects with an S3-compatible sync tool such as rclone.",
    "6. Update `wrangler.jsonc` bindings and run `pnpm deploy:dry-run` before deploy.",
    "",
  ].join("\n"),
);
