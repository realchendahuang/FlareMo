...
| 音频/文稿阅读 | 已实现：音频+转写同篇承载，吸顶播放器（倍速/拖动/进度记忆/跟随滚动）、标题锚点与可折叠目录、时间戳点击跳转、时间线长文折叠，详情页与分享页同享（见 R11） | 正文内嵌图片、分享页 SEO、ASR 自动转写（R4） |
...
- **状态**：✅ 已实现（2026-09-12）。同篇承载 + 吸顶播放器（倍速/拖动/进度记忆/跟随滚动）+ 文稿排版（标题锚点/可折叠目录/图片约束）+ 时间戳跳转 + 时间线长文折叠，详情页与公开分享页同享；纯前端展示层改动，零新表零 migration。后置项：正文内嵌图片、分享页 SEO。
