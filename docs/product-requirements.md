...
- **内容**：
  1. **memo 可选日期字段**：新增可选 `eventAt`（指向未来的某一天），编辑器提供一行「⏰ 加日期」入口；详情页可回填/修改/清除。
  2. **月历视图**：放在热力图（`activity-heatmap`）旁。过去格子按 `createdAt` 铺已写笔记，未来格子按 `eventAt` 铺已标日期的笔记；两种着色一眼可辨。
  3. **点日期筛选**：点热力图或月历的某一天 → 筛选当天时间线。
- **定位**：日历不是独立日程模块，是现有笔记数据的另一个时间切面，与热力图、每日回顾同一设计语言。没有独立 event/日程表，待办就是绑了日期的一条笔记。
- **价值**：中。给「安排未来」一个轻量出口，同时把过去的时间线多一个查看视角；不引入新范式。
- **成本**：低中。一条 migration + 一个过滤维度 + 一个月历组件；权限矩阵与 Memos 兼容 API 零改动（字段可选，导入导出按可选字段处理）。
- **依赖**：无。
...
- R10 中 `eventAt` migration 与月历组件彼此独立，适合穿插；权限矩阵零改动。
